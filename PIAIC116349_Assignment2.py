...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = arr-np.repeat(brr,3).reshape(3,3)
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.array([np.repeat(arr,3),np.tile(arr,3)]).ravel()
...
def function14():
    ...
    ans = np.split(arr.reshape(8,3),4,axis=0)
...
def function18():
    ...
    x = len(students[students<len(piaic)])
    return x
